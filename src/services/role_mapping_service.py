...
class RoleMappingService:
    ...
    async def _call_gemini_stream(
        ...
        """
        Stream role mapping generation from Gemini (yields chunks + final JSON)
        """
        try:
            PROMPT = ROLE_MAPPING_PROMPT_V5_STATE if organization_data["department_id"] else ROLE_MAPPING_PROMPT_V2
            output_json_format = state_json_output if organization_data["department_id"] else center_json_output
            ...
            if additional_document:
                contents[0].parts.insert(0, types.Part.from_bytes(data=additional_document, mime_type="application/pdf"))
            ...
            logger.error(f"Error streaming role mapping from Gemini: {str(e)}")
